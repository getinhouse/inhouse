 ...
 * DemoBrain after a short think-delay; speech uses the browser's own
 * speechSynthesis (local, free, no audio fetch). No request ever leaves
 * the page — the demo makes the same privacy promise as the product.
 */
...
export class DemoClient implements AssistantApi {
  ...
  async sendTextTurn(_sessionId: string, text: string): Promise<TurnResponse> {
    ...
    const thinkMs = Math.round(this.thinkMsFloor + reply.length * this.thinkMsPerChar);
    ...
    return {
      ...
      // speak() below instead of a WAV fetch.
  ...
  /** Speak via the browser's local TTS. Replaces the WAV stream fetch. */
